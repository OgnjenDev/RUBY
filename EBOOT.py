...
def bsod(auto_reboot=True, reboot_delay=5, error=None):
    ...
    Može da prikaže custom error kod.
    """
    # Dimenzije terminala
    try:
...
def measure_speed():
    ...
    try:
        response = requests.get(url, timeout=10)  # dodajemo timeout da ne visi
    except requests.RequestException:
        # Ako dođe do greške, pokreće se BSOD sa custom error kodom
        bsod(error="NETWORK_REQUEST_FAILED")
        return
